...
`fill-role`, `wait-role`, `click-role`, and verification steps before falling back to custom
JavaScript.
...
- Prefer semantic actions such as `wait-role`, `click-role`, `click-text`,
  `click-index`, `fill-label`, `fill-role`, `select-label`, and `check-label` before raw
