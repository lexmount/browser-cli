# browser-cli usable version status

This page summarizes the current mainline state for users and agents that need
to try `browser-cli` now.
...
- If and only if GitHub archive/codeload access blocks that exact command and
  `git --version` succeeds, use the official fallback:
  `uv tool install --force git+https://github.com/lexmount/browser-cli.git`.
  Coding-agent command approval is expected when policy requires it; do not
  bypass approval through a clone/local-path install or guess another package,
  repository, download host, or console domain.
...
Run these commands after installing and configuring credentials:
...
Treat the install as ready for browser work when:
...
- The smoke session reports `created=true` and `closed=true`.
...
- Do not paste API keys, access tokens, refresh tokens, or full direct browser
...
Until those pieces land, the supported trial flow is the Quickstart manual env
path:

- [Quickstart](./quickstart.md)
- [Connect from Codex site contract](./connect-from-codex.md)
