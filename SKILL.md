...
`click-index`, `fill-label`, `link-snapshot`, `table-snapshot`, `list-snapshot`, `text-snapshot`, `outline-snapshot`, `form-snapshot`,
...
`aria_live`, `headings`, `landmarks`, `outline_count`,
...
1. Inspect with `snapshot`, then `interactive-snapshot` when selectors or roles
   ...
   visible text, alerts, and status messages.
